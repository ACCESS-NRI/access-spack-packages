...
2.  **Package Classes**:
    *   **`Um` (`packages/um/package.py`)**: Inherits from `UmBasePackage`. It defines the specific model variants (e.g., `vn13`, `vn13p1-am`) and provides the logic for installing the main atmosphere and reconfiguration executables.
    *   **`UmCreatebc` (`packages/um_createbc/package.py`)**: Inherits from `UmBasePackage`. It manages the `um_createbc.exe` utility program, specifically used for the creation of lateral boundary conditions.
